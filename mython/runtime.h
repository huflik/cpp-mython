...
namespace detail {
    ...
    template<typename T, typename R, typename Operator>
    std::optional<R> BinaryOperation(const ObjectHolder& lhs, const ObjectHolder& rhs, Operator op) {
        T* lhs_ptr = lhs.TryAs<T>();
        T* rhs_ptr = rhs.TryAs<T>();
        if (rhs_ptr && lhs_ptr) {
            return op(lhs_ptr->GetValue(), rhs_ptr->GetValue());
        }
        return {};
    }
        
    template<typename T, typename Comparator>
    std::optional<bool> IsComparable(const ObjectHolder& lhs, const ObjectHolder& rhs, Comparator comparator) {
        return  BinaryOperation<T, bool, Comparator>(lhs, rhs, comparator);
    }

    template<typename Comparator>
    std::optional<bool> Compare(const ObjectHolder& lhs, const ObjectHolder& rhs, Comparator comparator) {
        if (auto result = IsComparable<Number>(lhs, rhs, comparator)) {
            return result.value();
        }
        if (auto result = IsComparable<String>(lhs, rhs, comparator)) {
            return result.value();
        }
        if (auto result = IsComparable<Bool>(lhs, rhs, comparator)) {
            return result.value();
        }
        return {};
    }
